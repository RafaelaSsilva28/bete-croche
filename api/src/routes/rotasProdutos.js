...
router.post('/produtos', autenticarToken, async (req, res) => {
    ...
    if (sob_encomenda === true && (!tempo_producao || tempo_producao.trim() === "")) {
        return res.status(400).json({
...
router.get('/produtos/:id_produto', async (req, res) => {
    ...
    try {
        ...
        const resultado = await BD.query(query, [id_produto]);
...
router.patch('/produtos/:id_produto', autenticarToken, async (req, res) => {
    ...
    try {
        ...
        // Se informou que é sob encomenda,
        // precisa informar o tempo de produção
        if (
            sob_encomenda === true &&
            (!tempo_producao || tempo_producao.trim() === "")
        ) {
            return res.status(400).json({
                message: "Informe o tempo de produção."
            });
        }
        ...
        if (sob_encomenda !== undefined) {
            campos.push(`sob_encomenda = $${contador}`);
            valores.push(sob_encomenda);
            contador++;
        }

        if (tempo_producao !== undefined) {
            campos.push(`tempo_producao = $${contador}`);
            valores.push(tempo_producao);
            contador++;
        }

        if (material !== undefined) {
            campos.push(`material = $${contador}`);
